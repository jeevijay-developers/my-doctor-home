// Standalone WhatsApp send endpoint — the delivery-abstraction boundary
// mentioned in the spec ("Create separate Edge Functions/services") and the
// function the "Send Test Reminder" button calls directly. The scheduled
// checkup-reminder-worker calls the same underlying sendWhatsApp() helper
// in-process (see _shared/notificationProviders.ts) rather than making an
// HTTP hop to this function, but both paths produce identical
// notification_logs rows either way.
import { createClient } from "npm:@supabase/supabase-js@2";
import { sendWhatsApp, corsHeaders, json } from "../_shared/notificationProviders.ts";
import { WHATSAPP_CHECKUP_TEMPLATE, renderTemplate } from "../_shared/notificationTemplates.ts";
...
Deno.serve(async (req) => {
  ...
  const checkupDate = body?.checkup_date
    ...
    : "your next visit";
  const message = body?.message || renderTemplate(WHATSAPP_CHECKUP_TEMPLATE, {
  ...
  const { data: log, error: insertErr } = await admin.from("notification_logs").insert({
    ...
    channel: "whatsapp", notification_type: isTest ? "test_reminder" : "manual", recipient: patient.phone,
    message, status: "pending", is_test: isTest,
  }).select("*").single();
  if (insertErr || !log) {
    console.error("send-whatsapp-notification: log insert failed", { code: insertErr?.code, message: insertErr?.message });
