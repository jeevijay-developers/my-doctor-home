-- get_or_create_dev_user used to create a brand-new dev.<phone>@doctylia.com
-- account for ANY phone number regardless of _is_signup — so logging in with
-- a number that had never signed up silently created a fresh "Test User"
-- account and dropped the caller straight into the dashboard, instead of
-- surfacing "No account found with this phone number." like the real SMS
-- OTP path (ProductionOtpService) already does. Only create on signup now;
-- a login lookup that finds nothing returns zero rows.
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    IF NOT _is_signup THEN
      -- Login attempt for a phone number with no dev account: return no
